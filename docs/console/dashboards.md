...
要管理儀表板，必須通過單擊`Dashboards`選單項來存取該部分：

![](~@cloud/MenuDashboards.jpg)

然後單擊`Add Dashboard`按鈕將打開新界面，以輸入儀表板詳細資訊，如以下畫面截圖所示：
...
在儀表板中啟用編輯模式後，將出現一個名為`Add Widget`的新按鈕。單擊它將顯示一個彈出視窗，可以在儀表板中選擇要新增的顯示小工具類型。有不同的小工具用於顯示資訊或控制連接的裝置，如下圖所示：
